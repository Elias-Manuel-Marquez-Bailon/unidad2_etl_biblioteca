-- ============================================
-- CONSULTAS DE VERIFICACION - ETL BIBLIOTECA
-- Base de datos: biblioteca_dw
-- ============================================

-- 1. Cuantos registros hay en fact_prestamos
SELECT COUNT(*) AS total_prestamos FROM fact_prestamos;

-- 2. Cuantos registros hay en etl_errores
SELECT COUNT(*) AS total_errores FROM etl_errores;

-- 3. Que errores fueron registrados
SELECT id_error, id_registro, descripcion_error, fila_csv, fecha_error
FROM etl_errores;

-- 4. Cual fue el ultimo estado registrado en etl_log
SELECT id_log, fecha_ejecucion, estado, filas_leidas, filas_cargadas, filas_rechazadas
FROM etl_log
...
SELECT l.libro, l.categoria, SUM(f.total_multa) AS total_multas
FROM fact_prestamos f
JOIN dim_libro l ON f.id_libro = l.id_libro
